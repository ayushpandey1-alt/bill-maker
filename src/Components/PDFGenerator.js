...
const PDFGenerator = ({ formData }) => {
   ...
   const buildPDF = () => {
    ...
    doc.setFontSize(9);
    doc.text(`GSTIN: 05AALFT3742K1ZW`, margin + 5, cursorY + 15);
    doc.setFontSize(12);
    doc.text("TAX INVOICE", pageWidth / 2 - 30, cursorY + 15);
    doc.setFontSize(9);
    doc.text("M/s. TRIDENT CHEMICALS", pageWidth / 2 - 50, cursorY + 35);
    doc.text(
      `Village-Toli, P.O. Panuwanaula, Distt.- Almora (Uttarakhand)`,
      pageWidth / 2 - 120,
      cursorY + 50
    );
    ...
    doc.setFont(undefined, "normal");
    doc.text(`By Road Truck`, pageWidth / 2 + 100, cursorY + 15);
    doc.setFont(undefined, "bold");
    ...
    doc.setFont(undefined, "normal");
    doc.text("PUNB0096200", margin + 56, cursorY + 90);
